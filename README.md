...
* 채팅 입력 영역에는 클릭 가능한 대화 기록 저장 스위치가 포함된다. 현재는 토글 상태만 보관한다.
* 채팅 입력 영역에는 추후 이미지나 파일 업로드에 사용할 비활성 `+` 첨부파일 액션이 포함된다.
* 입력창은 `Enter`로 전송하고 `Shift+Enter`로 줄바꿈한다.
* 긴 입력 텍스트는 입력창 폭에 맞춰 자동 줄바꿈된다.
* Header의 설정 아이콘을 클릭하면 설정 화면으로 전환된다.
...
* 모델 관리, 연동, 도움말, 피드백의 실제 기능은 추후 구현한다.
