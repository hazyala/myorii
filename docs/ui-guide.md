...
* Input Area: 채팅 기록 버튼, 첨부파일 액션, 대화 기록 저장 스위치, 텍스트 입력, 전송 버튼

현재 단계의 채팅 탭은 저장 스위치를 켠 대화의 로컬 기록 저장, 기록 목록 전환, 대화 이어가기, 삭제, 드래그 재정렬을 지원한다. 할일 탭은 로컬 저장, 체크 완료, 드래그 재정렬, 새 할 일 추가를 지원한다. 메모 탭은 로컬 저장, 목록/편집 화면 전환, 자동 저장, 삭제, 드래그 재정렬, Markdown 입력을 지원한다.
...
* 코드블록, 인라인 코드, 파일명/함수명/변수명/클래스명/명령어/Python·Java·HTML·CSS·C 계열 코드/SQL/짧은 단어 번역 후보는 클릭 시 복사되고 `복사됨` 토스트 표시
* 설명, 코드블록, 주의점이 섞인 Assistant 응답은 화면에서도 같은 순서로 표시
* 요약, 분석, 문서 내용 설명, 일상 대화는 사용자가 코드나 명령어를 요청하지 않는 한 일반 문장으로 표시
...
채팅 기록은 대화 기록 저장 스위치를 켠 대화만 저장한다. 메뉴바 아이콘 재클릭 또는 Header의 `×` 버튼으로 창을 숨기면 다시 열 때 항상 새 대화로 시작한다. 스위치가 꺼진 임시 대화는 기록에 남지 않는다.

채팅 기록 버튼을 클릭하면 채팅 탭 전체가 기록 목록으로 전환된다. 목록 항목은 첫 사용자 메시지 또는 첨부 파일명을 바탕으로 자동 생성된 제목을 표시한다. 항목을 클릭하면 저장된 메시지를 채팅 화면에 다시 렌더링하고 같은 세션에 이어서 새 메시지를 저장한다.

목록 항목 좌측에는 드래그 핸들을, 우측에는 `x` 삭제 버튼을 둔다. 드래그를 마치면 현재 UI 순서대로 `chat_sessions.ord`를 다시 저장하고, `x`를 누르면 해당 세션과 메시지/첨부 기록을 함께 삭제한다.
...
* 하나의 언어 코드 스니펫, SQL, 셸 스크립트는 줄바꿈과 들여쓰기를 유지한 하나의 코드블록으로 유지해 전체 복사
* 일반 요약/분석/일상 대화는 복사 카드로 자동 승격하지 않음
