...
채팅 기록은 대화 기록 저장 스위치를 켠 대화만 저장한다. 스위치가 꺼진 임시 대화는 메뉴바 아이콘 재클릭 또는 Header의 `×` 버튼으로 창을 숨길 때 삭제되며, 다시 열면 새 대화로 시작한다.
...
목록 항목 우측에는 드래그 핸들과 `x` 삭제 버튼을 둔다. 드래그를 마치면 현재 UI 순서대로 `chat_sessions.ord`를 다시 저장하고, `x`를 누르면 해당 세션과 메시지/첨부 기록을 함께 삭제한다.
