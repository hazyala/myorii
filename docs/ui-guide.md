...
첨부 파일은 `jpg`, `jpeg`, `png`, `txt`, `md`, `csv`, `json`, `pdf`, `doc`, `docx`, `hwp`, `hwpx`, `xls`, `xlsx`, `ppt`, `pptx` 등 확장자 기반으로 지원한다. 지원하지 않는 파일을 선택하거나 드롭하면 Assistant 메시지 영역에 지원하지 않는 파일 형식 오류를 표시한다.
...
* 입력창 항상 하단 고정
* 코드 블록 원클릭 복사
* 실제 채팅 내용은 Ollama `qwen3-vl:4b` 모델 응답으로 렌더링
