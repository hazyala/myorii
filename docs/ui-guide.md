...
* Header: Myorii 캐릭터, 앱 이름, 온라인/오프라인 상태, 설정 아이콘
* Tab: 채팅, 할일, 메모 3개 탭
* Content Area: 선택된 탭에 맞는 화면 표시
* Chat Area: Ollama 연동 전까지 비어 있는 대화 영역과 입력 영역만 표시
* Input Area: 대화 기록 저장 스위치, 텍스트 입력, 전송 버튼

현재 단계의 할일/메모 탭은 선택 시 각 빈 화면으로 전환된다.
...
* Ollama 연동 전까지 대화 내용 비움
* 추후 사용자 메시지와 AI 응답 표시
