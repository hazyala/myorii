...
현재 모델 입력까지 실제 본문을 전달하는 첨부 형식은 이미지 파일이다. `TextHandler`는 `txt`, `md`, `json`, `yaml`, `yml` 본문 일부를 추출할 수 있고, `CsvHandler`는 `csv`, `tsv` 컬럼명과 샘플 행을 요약할 수 있지만 아직 요청 경로에는 연결하지 않았다. 문서, 스프레드시트, 프레젠테이션은 UI 첨부와 파일명 전달까지만 구현되어 있으며, 본문 파싱과 요약 전달은 후속 `AttachmentRouter` 단계에서 구현한다.
