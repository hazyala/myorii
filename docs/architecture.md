...
파일 첨부는 `ChatView`가 전송 전 로컬 파일 경로를 입력부 상태로 보관하고, 입력창 위 미리보기 카드로 표시한다. 전송 시 화면에는 첨부 파일을 사용자 메시지 버블 본문에 섞지 않고 버블 상단 바깥의 미리보기 카드로 렌더링한다. 전송 후 첨부 카드가 한 줄 폭을 넘으면 가로로 넘치지 않고 다음 줄로 재배치한다. LLM 요청 텍스트에는 첨부 파일명을 함께 전달한다. 이미지 첨부는 `ChatAttachmentPayload`로 `ChatWorker`, `ChatService`, `OllamaClient`까지 전달하고, `ImageHandler`가 base64로 변환해 Ollama 요청의 `messages[].images`에 포함한다. `AttachmentRouter`는 비이미지 첨부를 handler에 위임한다. `TextHandler`는 `txt`, `md`, `json`, `yaml`, `yml` 첨부 본문 일부를 `AttachmentContext`로 추출하고, `CsvHandler`는 `csv`, `tsv` 첨부의 컬럼명과 샘플 행을 `AttachmentContext`로 요약한다. `ChatService`는 추출된 context를 user message 본문과 metadata에 추가해 모델 요청에 포함한다. 지원 파일은 확장자 기준 UI 선택/미리보기 범위로 판별하고, 지원하지 않는 파일은 Assistant 메시지로 오류를 렌더링한다. PDF, 문서, 스프레드시트, 프레젠테이션 본문 파싱은 아직 구현되지 않았으며 `AttachmentRouter`의 후속 handler에서 처리한다.
