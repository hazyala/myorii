...
따라서 모든 요청을 하나의 긴 프롬프트로 처리하지 않는다. 로컬 규칙은 요청 의도, 프롬프트 프로필, 응답 포맷을 고르는 데 사용하고, 사용자에게 표시되는 채팅 답변은 사용자가 지정한 단일 Ollama 모델 `qwen3-vl:4b`로 생성한다.
...
모든 요청은 사용자가 선택한 단일 모델로 보낸다. V1 기준 기본 모델은 `qwen3-vl:4b`이며, 텍스트와 이미지 요청을 모두 같은 모델에서 처리한다.
...
```text
ChatService.warmup()
  -> OllamaClient.warmup(qwen3-vl:4b)
