...
| CHAT-HISTORY-005 | 기록 재정렬 | 미실행 | 채팅 기록 목록 좌측 핸들 드래그 | 드래그한 순서가 목록을 닫았다 다시 열어도 유지되는지 확인 |
| CHAT-HISTORY-006 | 닫기 후 새 대화 | 미실행 | 저장된 기록을 열어 확인한 뒤 메뉴바 아이콘 재클릭 | 창을 다시 열었을 때 이전 세션이 이어지지 않고 빈 새 대화로 시작하는지 확인 |
