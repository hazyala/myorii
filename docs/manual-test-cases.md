...
| RESPONSE-NORMAL-007 | 코드 생성 | 성공 | 파이썬 코드 줘 리스트 중복 제거 | Python 함수 코드가 단일 복사 블록으로 표시되고 들여쓰기 유지 확인 |
| RESPONSE-NORMAL-008 | 코드 생성 | 성공 | 자바 코드 줘 사용자 클래스 | Java 클래스 코드가 단일 복사 블록으로 표시되고 줄바꿈/들여쓰기 유지 확인 |
| RESPONSE-NORMAL-009 | 코드 생성 | 성공 | HTML 코드 줘 로그인 폼 | HTML 문서 코드가 단일 복사 블록으로 표시되고 태그 줄바꿈/들여쓰기 유지 확인 |
| RESPONSE-NORMAL-010 | 코드 생성 | 성공 | CSS 코드 줘 버튼 스타일 | CSS 버튼 스타일 코드가 단일 복사 블록으로 표시되고 줄바꿈/들여쓰기 유지 확인 |
| RESPONSE-NORMAL-011 | 코드 생성 | 성공 | C언어 코드 줘 hello world | C hello world 코드가 단일 복사 블록으로 표시되고 줄바꿈/들여쓰기 유지 확인 |
| RESPONSE-NORMAL-012 | SQL 생성 | 성공 | 활성 사용자 조회 SQL문 줘 | SQL 조회문이 복사 가능한 블록으로 표시됨 |
| RESPONSE-NORMAL-013 | 명령어 | 성공 | 터미널에서 현재 폴더 파일 목록 보는 명령어 알려줘 | `bash` 문자열 오분리 없이 실제 명령어 `ls`, `ls -a`가 후보별 복사 블록으로 표시됨 |
...
| ID | 영역 | 테스트 상태 | 테스트 프롬프트 | 확인 내용 |
| --- | --- | --- | --- | --- |
| CHAT-HISTORY-001 | 저장 스위치 | 미실행 | 대화 기록 저장 스위치를 켠 뒤 메시지 전송 | 첫 사용자 메시지 기반 제목으로 채팅 기록 목록에 세션이 생성되는지 확인 |
| CHAT-HISTORY-002 | 임시 대화 폐기 | 미실행 | 저장 스위치를 끄고 메시지 전송 후 메뉴바 아이콘 재클릭 | 창을 다시 열었을 때 이전 임시 대화가 사라지고 새 대화로 시작하는지 확인 |
| CHAT-HISTORY-003 | 대화 이어가기 | 미실행 | 저장된 기록 항목 클릭 후 추가 메시지 전송 | 이전 메시지가 렌더링되고 같은 세션에 새 메시지가 이어서 저장되는지 확인 |
| CHAT-HISTORY-004 | 기록 삭제 | 미실행 | 채팅 기록 목록에서 `x` 클릭 | 해당 세션이 목록에서 제거되고 다시 열리지 않는지 확인 |
| CHAT-HISTORY-005 | 기록 재정렬 | 미실행 | 채팅 기록 목록 우측 핸들 드래그 | 드래그한 순서가 목록을 닫았다 다시 열어도 유지되는지 확인 |
| CHAT-HISTORY-006 | 저장된 대화의 임시 꼬리 | 미실행 | 저장된 기록을 연 뒤 스위치를 끄고 추가 메시지 전송 후 창 닫기 | 다시 열었을 때 스위치를 끈 뒤 보낸 메시지는 저장되지 않고 기존 저장 지점까지만 남는지 확인 |
