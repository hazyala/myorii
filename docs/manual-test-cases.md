...
| RESPONSE-NORMAL-005 | 네이밍 | 성공 | 사용자 프로필 변수 3개만 추천해줘 | `변수명` 대신 `변수`라고 요청해도 변수명 후보 3개가 후보별 코드블록으로 분리 표시됨 |
| RESPONSE-NORMAL-006 | 짧은 단어 번역 | 성공 | 단어 사과 영어로 뭐야? | 짧은 단어 번역 결과가 복사 가능한 단일 블록으로 표시됨 |
| RESPONSE-NORMAL-007 | 문장 번역 | 성공 | 처음 뵙겠습니다 영어로 번역해줘 | 새 세션에서 `Nice to meet you.` 일반 문장 응답 확인. 같은 세션 연속 테스트 중 빈 응답 오류가 1회 발생해 세션/입력 용량 안정화는 로드맵에서 별도 추적 |
| RESPONSE-NORMAL-008 | 짧은 단어 번역 코드펜스 보정 | 성공 | 토끼 영어로 | 모델이 `text` 코드펜스나 `English:` 라벨을 포함해도 UI에는 실제 번역어만 단일 복사 블록으로 표시됨 |
| RESPONSE-NORMAL-009 | 코드 생성 | 성공 | 파이썬 코드 줘 리스트 중복 제거 | Python 함수 코드가 단일 복사 블록으로 표시되고 들여쓰기 유지 확인 |
| RESPONSE-NORMAL-010 | 코드 생성 | 성공 | 자바 코드 줘 사용자 클래스 | Java 클래스 코드가 단일 복사 블록으로 표시되고 줄바꿈/들여쓰기 유지 확인 |
| RESPONSE-NORMAL-011 | 코드 생성 | 성공 | HTML 코드 줘 로그인 폼 | HTML 문서 코드가 단일 복사 블록으로 표시되고 태그 줄바꿈/들여쓰기 유지 확인 |
| RESPONSE-NORMAL-012 | 코드 생성 | 성공 | CSS 코드 줘 버튼 스타일 | CSS 버튼 스타일 코드가 단일 복사 블록으로 표시되고 줄바꿈/들여쓰기 유지 확인 |
| RESPONSE-NORMAL-013 | 코드 생성 | 성공 | C언어 코드 줘 hello world | C hello world 코드가 단일 복사 블록으로 표시되고 줄바꿈/들여쓰기 유지 확인 |
| RESPONSE-NORMAL-014 | SQL 생성 | 성공 | 활성 사용자 조회 SQL문 줘 | SQL 조회문이 복사 가능한 블록으로 표시됨 |
| RESPONSE-NORMAL-015 | 명령어 | 성공 | 터미널에서 현재 폴더 파일 목록 보는 명령어 알려줘 | `bash` 문자열 오분리 없이 실제 명령어 `ls`, `ls -a`가 후보별 복사 블록으로 표시됨 |
