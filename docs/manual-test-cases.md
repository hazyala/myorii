...
| RESPONSE-NORMAL-013 | 명령어 | 이슈 발견 | 터미널에서 현재 폴더 파일 목록 보는 명령어 알려줘 | 실제 명령어 `ls`는 복사 블록으로 표시되나, `bash` 문자열이 별도 복사 블록으로 함께 표시됨 |
...
| RESPONSE-EDGE-005 | Markdown 렌더링 순서 | 이슈 발견 | 설명, 코드, 주의점이 함께 있는 응답 생성 | 설명-코드-주의점 순서는 유지됐으나, 요청한 Python 코드블록이 일반 텍스트로 풀려 표시됨 |
