...
| ATTACH-FIX-007 | 첨부 UX | ATTACH-EDGE-003 | 첨부 테스트 중 파일을 드래그할 때 대상 영역 또는 대화 영역이 자동 스크롤되지 않아 조작이 불편함 | 드래그 중 포인터가 스크롤 영역 상단/하단에 가까워지면 자동 스크롤되어 원하는 위치로 이동할 수 있어야 함 | 미수정 |
...
| ATTACH-FIX-011 | 드래그 이벤트 | ATTACH-EDGE-011, ATTACH-EDGE-012 | 미지원 파일을 드래그하는 동안 정확히 드롭하지 않았는데도 "지원하지 않는 파일 형식입니다" 오류 채팅이 20회 이상 반복 생성됨. `unsupported.exe`, `unsupported.zip`에서 재현 | 드롭이 확정된 시점에만 오류를 1회 표시하고, drag enter/move/hover 단계에서는 채팅 메시지를 생성하지 않아야 함 | 미수정 |
...
| 드래그 자동 스크롤 | 첨부 전반 | 파일 드래그 중 스크롤 영역 가장자리에 접근하면 자동으로 위/아래 스크롤되어 긴 대화에서도 첨부 조작이 쉬워야 함 |
