...
| ATTACH-FIX-003 | context 한도 | ATTACH-NORMAL-010 | `sample.jpeg` 첨부 요청에서 `request (4921 tokens) exceeds the available context size (4096 tokens)` 원시 API 오류가 노출됨 | 원시 오류 대신 메시지 버블에 사용자용 안내를 표시하고, 대화 누적 초과와 첨부 용량 초과를 구분해야 함 | 수정 완료 |
...
| context 한도 초과 | ATTACH-NORMAL-010, ATTACH-EDGE-002 | 첨부는 최대 5개/총 5MB로 사전 제한하고, 요청 중 초과 시 대화 누적과 첨부 용량 초과를 구분해 사용자용 메시지를 표시 |
...
* ATTACH-FIX-003: Ollama context 초과 raw error를 사용자용 메시지로 변환하고, 마지막 사용자 요청에 첨부가 있으면 첨부 용량 초과 안내, 첨부가 없으면 긴 대화 안내를 표시하도록 분리했다. 첨부 추가 단계에서도 파일은 최대 5개, 총 5MB까지만 허용해 과도한 첨부 요청을 사전에 차단한다. 모델이 토큰을 반환하지 않고 종료하는 경우 빈 Assistant 버블 대신 재시도 안내를 표시한다.
