...
| ATTACH-FIX-009 | 응답 안정성 | 첨부 전반 | 채팅이 길어지거나 첨부파일이 여러 개 누적되면 응답 내용이 중복되거나 무관한 내용, 깨진 코드블록, 응답 용량 초과 오류 등으로 불안정해짐 | 긴 대화와 첨부파일 상황에서도 히스토리/첨부파일 참고 내용을 안정적으로 관리하고, 필요 시 새 대화 안내나 히스토리 축약을 적용해야 함 | 부분 수정 |
